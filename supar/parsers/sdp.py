...
from supar.models.sdp import TriaffineSemanticDependencyModel, DyGLSemanticDependencyModel
...
class DyGLSemanticDependencyParser(Parser):
    r"""
    The implementation of Graph Structure Learning for Semantic Dependency Parser.
    """

    NAME = 'dygl-semantic-dependency'
    MODEL = DyGLSemanticDependencyModel
    ...
    def load(cls, path, reload=False, src=None, **kwargs):
        ...
        Examples:
            >>> from supar import Parser
            >>> parser = Parser.load('gsl-sdp')
            >>> parser = Parser.load('./dm.gsl.sdp.lstm.char')
        """
    ...
    def _train(self, loader):
        ...
        for i, batch in enumerate(bar, 1):
            ...
            mask = mask.unsqueeze(1) & mask.unsqueeze(2)
            mask[:, 0] = 0
            s_edge, s_label, s_adj = self.model(words, feats)
            loss = self.model.loss(s_edge, s_label, s_adj, labels, mask)
            loss = loss / self.args.update_steps
            loss.backward()
            nn.utils.clip_grad_norm_(self.model.parameters(), self.args.clip)
            if i % self.args.update_steps == 0:
    ...
    def _evaluate(self, loader):
        ...
        for batch in loader:
            ...
            mask = mask.unsqueeze(1) & mask.unsqueeze(2)
            mask[:, 0] = 0
            s_edge, s_label, s_gsl = self.model(words, feats)
            loss = self.model.loss(s_edge, s_label, s_gsl, labels, mask)
            total_loss += loss.item()
