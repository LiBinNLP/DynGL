...
class DyGLSemanticDependencyModel(Model):
    r"""
    The implementation of Graph Structure Learning Semantic Dependency Parser.
    ...
    def __init__(self,
        ...
        self.gsl_mlp_d = MLP(n_in=self.args.n_lstm_out, n_out=self.args.n_edge_mlp, dropout=edge_mlp_dropout, activation=False)
        self.gsl_mlp_h = MLP(n_in=self.args.n_lstm_out, n_out=self.args.n_edge_mlp, dropout=edge_mlp_dropout, activation=False)
        self.gsl_attn = Biaffine(n_in=self.args.n_edge_mlp, n_out=2, bias_x=True, bias_y=True)

        self.edge_mlp_d = MLP(n_in=self.args.n_lstm_out+n_edge_mlp, n_out=n_edge_mlp, dropout=edge_mlp_dropout, activation=False)
        self.edge_mlp_h = MLP(n_in=self.args.n_lstm_out+n_edge_mlp, n_out=n_edge_mlp, dropout=edge_mlp_dropout, activation=False)
        self.label_mlp_d = MLP(n_in=self.args.n_lstm_out+n_edge_mlp, n_out=n_label_mlp, dropout=label_mlp_dropout, activation=False)
        self.label_mlp_h = MLP(n_in=self.args.n_lstm_out+n_edge_mlp, n_out=n_label_mlp, dropout=label_mlp_dropout, activation=False)
    ...
    def forward(self, words, feats=None):
        ...
        context_embedding = self.lstm_encode(words, feats)

        # graph structure learning
        gsl_d = self.gsl_mlp_d(context_embedding)
        gsl_h = self.gsl_mlp_h(context_embedding)
        # [batch_size, seq_len, seq_len, 2]
        s_adj = self.gsl_attn(gsl_d, gsl_h).permute(0, 2, 3, 1)

        # graph representation learning (graph neural networks)
        adj = s_adj.argmax(-1).lt(1)
        adj = torch.where(adj, 1.0, 0.0)
        node_embedding = self.graph_encoder(context_embedding, adj)
        ...
        return s_edge, s_label, s_adj

    def loss(self, s_edge, s_label, s_adj, labels, mask):
        r"""
        Args:
            s_edge (~torch.Tensor): ``[batch_size, seq_len, seq_len, 2]``.
                Scores of all possible edges.
        ...
        edge_mask = labels.ge(0) & mask
        gsl_loss = self.criterion(s_adj[mask], edge_mask[mask].long())
        edge_loss = self.criterion(s_edge[mask], edge_mask[mask].long())
        label_loss = self.criterion(s_label[edge_mask], labels[edge_mask])
        return self.args.interpolation_a * gsl_loss + \
        self.args.interpolation_b * label_loss + (1 - self.args.interpolation_a - self.args.interpolation_b) * edge_loss
