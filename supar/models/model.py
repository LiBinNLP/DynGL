...
class Model(nn.Module):
    ...
    def __init__(self,
        ...
        if encoder != 'bert':
            ...
            if 'elmo' in feat:
                ...
                n_feat_input += self.elmo_embed.n_out
            if 'bert' in feat:
                ...
                n_feat_input += self.bert_embed.n_out
            self.embed_dropout = IndependentDropout(p=embed_dropout)

            # sdp lstm
            if encoder == 'lstm':
                self.lstm_encoder = VariationalLSTM(input_size=n_input,
                                                   hidden_size=n_lstm_hidden,
                                                   num_layers=n_lstm_layers,
                                                   bidirectional=True,
                                                   dropout=encoder_dropout)
                self.encoder_dropout = SharedDropout(p=encoder_dropout)
                self.args.n_lstm_out = n_lstm_hidden * 2
            else:
        ...
        if self.args.gnn_module == 'GCN':
            gnn_module = GCN(
                nfeat=self.args.n_lstm_out,
                nhid=self.args.n_gnn_hidden,
                nclass=self.args.n_edge_mlp,
            ...
            gnn_module = GAT(
                in_dim=self.args.n_lstm_out,
                hidden_size=self.args.n_gnn_hidden,
                mem_dim=self.args.n_edge_mlp,
                gat_dropout=self.args.encoder_dropout,
                num_layers=self.args.graph_hops)
    ...
    def lstm_encode(self, words, feats):
